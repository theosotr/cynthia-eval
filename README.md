...
You are expected to see something like the following
before entering the shell of the container.

```bash
Initialize Backends
 * Starting MSSQL database server
nohup: appending output to 'nohup.out'
 * Starting MySQL database server mysqld   No directory, logging in with HOME=/    [ OK ]
 * Starting PostgreSQL 10 database server   [ OK ]
cynthia@7062f213e4f6:~$
...
Replay the execution of the testing session named 'Cucumbers',
and run ORM queries on MySQL instead of Postgres.
...
Replay the execution of the testing session named 'Cucumbers',
and differentially test `SQLAlchemy` and `Sequelize`
instead of `Django` and `peeewee`.
...
which is also provided by the user
(i.e., not generated by `Cynthia`).
...
`cynthia generate` generates a number of relational database schemas,
...
Generate 5 random database schemas.
For every schema, generate 100 AQL queries.
The generated data are generated by an SMT solver,
...
```bash
cynthia@0fbedf262c3d:~$ diff --color .cynthia/sessions/Cucumbers/73/sequelize_sqlite.out \
...
and all the tables and databases previously created
...
## Reproducing the two motivating ORM bugs

Now, we will use `Cynthia` to reproduce the
two bugs mentioned in Section II of our paper. 
...
that trigger the Django and peewee bugs mentioned in the paper (Section II).
...
```bash
cynthia@8461308e0af8:~$ setup-orms.sh --django-version 3.1
```

This script will install the buggy Django version that contains the bug
we want to reproduce (i.e., version 3.1).
After that, run `Cynthia` as follows
...
The command above means that we run the AQL query
...
```
Running Schemadb 100% [=========================== Passed ✔: 0, Failed ✘: 1, Unsp: 0, Timeouts: 0
Command run finished successfully.
```
...
```bash
cynthia@8461308e0af8:~$ diff --color .cynthia/sessions/Schemadb/1/django_mysql.out \
...
```bash
cynthia@8461308e0af8:~$ setup-orms.sh --peewee-version 3.13.3
```
...
```bash
cynthia@8461308e0af8:~$ diff --color .cynthia/sessions/Schemadb/1/django_sqlite.out \
...
in Section II of our paper (Figure 3).
...
The first volume (`bugs/`) contains the bug database
that includes the bugs discovered by our approach,
while the second volume (`scripts/`) includes some scripts
to reproduce and validate the results of the paper.
...
This database is initialized based on
the SQL script stored into `bugs/bug_schema.sql`.
...
* `issue_url`: A URL pointing to the bug report opened by us.
...
Get the number of release blockers

```bash
cynthia@2c16ea0e6849:~$ sqlite3 bugs/bugdb.sqlite3 \
   "SELECT COUNT(*) FROM ORM_BUG WHERE release_blocker = 1"
3
```
...
Note that one bug affects more than one ORM feature.
This is why the sum by column is greater than
...
containing test cases for reproducing
every ORM bug
discovered by our approach
and included in the artifact.
Each test case consists of

* `bugs/test-cases/<Bug ID>/TestSchema.sql`:
  The database schema on which we run the test case.
* `bugs/test-cases/<Bug ID>/query.aql.json`:
  The AQL query that triggers the ORM bug identified by the ID `<BUG ID>`.
* `bugs/test-cases/<Bug ID>/query.aql`:
  The AQL (in readable format) that triggers the ORM bug identified by the ID `<BUG ID>`.
...
In particular, the script `reproduce-bug.sh` performs
the following query on our bug database

```sql
SELECT orm, orm_version, affected_backend
FROM ORM_BUG
WHERE id = <BUG ID>
```
